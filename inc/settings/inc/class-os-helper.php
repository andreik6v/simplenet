...
class Helper {
	...
	public static function create_child_theme() {
		...
		$henry_dir            = get_template_directory();
		$henry_child_dir      = str_replace( '/themes/henry', '/themes/henry-child', $henry_dir );

		// Create directory.
		if ( ! file_exists( $henry_child_dir ) ) {
			wp_mkdir_p( $henry_child_dir );
		}

		// Copy CSS file.
		if ( ! copy( $child_theme_file_dir . '/style.css', $henry_child_dir . '/style.css' ) ) {
			error_log( 'Failed to copy style.css.' );
		}

		// Copy screenshot.
		if ( ! copy( $child_theme_file_dir . '/screenshot.png', $henry_child_dir . '/screenshot.png' ) ) {
			error_log( 'Failed to copy screenshot.png.' );
		}

		// Copy functions.php file.
		if ( ! copy( $child_theme_file_dir . '/functions.php', $henry_child_dir . '/functions.php' ) ) {
			error_log( 'Failed to copy functions.php.' );
		}

		// Activate child theme.
		switch_theme( 'henry-child' );
	}
	...
	public function set_theme_styles( $theme_json ) {
		...
		$henry_style   = json_decode( file_get_contents( get_template_directory() . '/styles/' . $settings['style'] . '.json' ) );
		$henry_palette = $henry_style->settings->color->palette;

		// Change brand color.
		if ( isset( $settings['brand_color'] ) && $settings['brand_color'] != '' ) {
			$henry_palette[0]->color = $settings['brand_color'];
		}

		// Convert values for the filter.
		foreach ( $henry_palette as $key => $value ) {
			$henry_palette[ $key ] = (array) $value;
		}
